...
try:
    ...
    os.execl(sys.executable, 'python3', __file__, *sys.argv[1:]) if platform.system() == "Linux" else os.system("cls") if platform.system() == "Windows" else print("未知操作系统，可能需要重新运行！")
...
class YiBan:
    ...
    def notify(self,content):
        ...
        if self.dic['mail'] != "":
            ...
            return self.sendPushPlus(content)
        else:
            print(f"{string}未配置通知方式，取消发送！\n")
    ...
    def sendMail(self,content):
        try:
            ...
            server.sendmail(admin['mail']['sendMail'],[self.dic['mail']],msg.as_string())  # 括号中对应的是发件人邮箱账号、收件人邮箱账号、发送邮件
            server.quit()  # 关闭连接
            print(f"{content}电子信件发送成功！\n")
    ...
    def sendPushPlus(self,content):
        ...
        if self.dic['pushToken'] != "":
            ...
            if response['code'] == 200:
                ...
                print(f"Push Plus发信失败！\t失败原因:{response['msg']}\n")
