...
try:
    ...
    print("缺少toml依赖！请手动安装")
    print("命令:pip3 install toml -i https://pypi.tuna.tsinghua.edu.cn/simple")
    exit(1)
        ...
        try:
            ...
            <div style="background:linear-gradient(to right,#cccc,white);; width: 95%; max-width: 800px; margin: auto auto; border-radius: 5px; border:skyblue 2px solid; overflow: hidden; -webkit-box-shadow: 0px 0px 20px 0px rgba(0, 0, 0, 0.12); box-shadow: 0px 0px 20px 0px rgba(0, 0, 0, 0.18); font-family : YouYuan;">
                ...
                <p style="position: relative;color: white;background:#7c7676;padding: 5px 10px;margin: 15px auto 0 ;">泰拉瑞亚是冒险之地！是神秘之地！是可让你塑造、捍卫、享受的大地。在泰拉瑞亚，你有无穷选择。手指发痒的动作游戏迷？建筑大师？收藏家？探险家？每个人都能找到自己想要的。</p>
                    <div style="float:left; width:50%;margin-top: 18.5px;">
                      ...
                      <div style="width: 45%;float: right;margin-top: 18.5px;">
                        ...
                        <span>UID:<font style="text-decoration: none;color: orange;text-align: left;">{self.name}</font> <br>状态:<font style="text-decoration: none;color: orange ">{text}</font></span>
...
async def asyncMain():
    ...
    data = readToml()                   # 读取配置文件中的account以及admin字段
    if data:
        account = data['account']
        admin = data['admin']
    data = accountEnv()
    if data:                            # 如果环境变量存在账号密码，则追加到账号列表
        account.extend(data)
    data = adminEnv()
    if data:                            # 如果环境变量存在邮箱参数，则替换，环境变量优先级高于配置文件
        admin = data
    if account:                         # 只需存在账号数据，即可运行
        tasks = [asyncio.ensure_future(AioYiBan(each,admin).start()) for each in account if each]
        # 此处应当使用 asyncio.create_task() 但3.6.8版本不支持
